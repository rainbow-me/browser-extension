export const TEST_VARIABLES = {
  // anvil wallet index 0. blocked by some swaps providers.
  SEED_WALLET: {
    ADDRESS: '0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266',
    SECRET: 'test test test test test test test test test test test junk',
    PK: '0xac0974bec39a17e36ba4a6b4d238ff944bacb478cbed5efcae784d7bf4f2ff80',
  },
  // anvil wallet index 9.
  SWAPS_WALLET: {
    ADDRESS: '0xa0Ee7A142d267C1f36714E4a8F75612F20a79720',
    PK: '0x2a871d0798f97d79848a013d4936a73bf4cc922c825d33c1cf7073dff6d409c6',
  },
  EMPTY_WALLET: {
    ADDRESS: '0x3637f053D542E6D00Eee42D656dD7C59Fa33a62F',
    SECRET:
      'fragile height cinnamon brief various inform thing gorilla decrease enemy dream split',
    PK: '0x30cf0b4e692056ce62de7b9a9eb40e97feaf1ee191ac22b33346fae13ae2e61b',
  },
  PRIVATE_KEY_WALLET: {
    ADDRESS: '0x38eDa688Cd8DFC6FeE8016c85803a584A0564dDC',
    SECRET:
      '0xaeb5635a53c33d3c0d92c32881d7613ee9fe18be77772054f5e025bc4fa8c851',
  },
  PRIVATE_KEY_WALLET_2: {
    ADDRESS: '0x70997970C51812dc3A010C7d01b50e0d17dc79C8',
    SECRET:
      '0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d',
  },
  PRIVATE_KEY_WALLET_3: {
    ADDRESS: '0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC',
    SECRET:
      '0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a',
  },
  WATCHED_WALLET: {
    PRIMARY_ADDRESS: 'djweth.eth',
    SECONDARY_ADDRESS: 'brdy.eth',
  },
  DAPP_RECIPIENT: {
    ADDRESS: '0x2f318C334780961FB129D2a6c30D0763d9a5C970',
  },
  SEED_PHRASE_24: {
    SECRET:
      'pattern timber trouble emotion promote dignity south screen black resource churn noise famous junk gym slam author sense perfect shoulder disorder company rain spawn',
    ADDRESS: '0x9979527b7ff3f05BC08800A0Dc8d216f080bA016',
  },
};

// Standard native asset address used across all chains
const NATIVE_ASSET_ADDRESS = '0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE';

export const SWAP_VARIABLES = {
  DAI_ARBITRUM_ID: '0xda10009cbd5d07dd0cecc66161fc93d7c9000da1_42161',
  DAI_MAINNET_ADDRESS: '0x6b175474e89094c44da98b954eedeac495271d0f',
  DAI_MAINNET_ID: '0x6b175474e89094c44da98b954eedeac495271d0f_1',
  ETH_MAINNET_ID: `${NATIVE_ASSET_ADDRESS}_1`,
  ETH_OPTIMISM_ID: `${NATIVE_ASSET_ADDRESS}_10`,
  GMX_ARBITRUM_ID: '0xfc5a1a6eb076a2c7ad06ed22c90d7e710e35ad0a_42161',
  // POL has its own native token address on Polygon, not the standard native address
  POL_POLYGON_ID: '0x0000000000000000000000000000000000001010_137',
  OP_OPTIMISM_ID: '0x4200000000000000000000000000000000000042_10',
  UNI_BNB_ID: '0xbf5140a22578168fd562dccf235e5d43a02ce9b1_56',
  USDC_ARBITRUM_ID: '0xaf88d065e77c8cc2239327c5edb3a432268e5831_42161',
  USDC_MAINNET_ID: '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48_1',
  USDC_MAINNET_ADDRESS: '0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48',
  WBTC_MAINNET_ID: '0x2260fac5e5542a773aa44fbcfedf7c193bc2c599_1',
  ZEROX_MAINNET_ID: '0xe41d2489571d322189246dafa5ebde1f4699f498_1',
  REKT_MAINNET_ID: '0x4f8b986ecffe7bed5dbeb2b49310fb00ca85a539_1',
};

export const HARDWARE_WALLETS = {
  WALLET_1: '0x2419EB3D5E048f50D386f6217Cd5033eBfc36b83',
  WALLET_2: '0x37bD75826582532373D738F83b913C97447b0906',
};
