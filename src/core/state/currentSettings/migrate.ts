...
> = {
  ...
  'rainbow.zustand.currentTheme': (k, v) => {
    const storageSchema = wrapStorageSchema(
      z.object({
        currentTheme: z.string(),
        currentUserSelectedTheme: z.string(),
      }),
    );
    const parsedValue = storageSchema.parse(JSON.parse(v));

    // Note: This only migrates the currentTheme property
    // currentUserSelectedTheme will use its default value
    return ['settings:currentTheme', parsedValue.state.currentTheme];
  },
