...
import { persistStorage as rainbowStorage } from './persistStorage';
...
export interface RainbowPersistConfig<S, PersistedState = Partial<S>> {
  ...
  storageKey: string;
  /**
   * Whether to use the Rainbow naming schema for the persisted store.
   * @default true
   */
...
>(
  ...
  const { persistStorage, version } = createPersistStorage<S, PersistedState>({
    ...persistConfig,
    useRainbowNamingSchema: persistConfig.useRainbowNamingSchema ?? true,
  });
 ...
 */
export function omitStoreMethods<S, PersistedState extends Partial<S>>(
  state: S,
): PersistedState {
...
const DEFAULT_PERSIST_THROTTLE_MS = 5_000;
...
function createPersistStorage<S, PersistedState extends Partial<S>>(
  ...
  const lazyPersist = debounce(
    async function persist(
      ...
      try {
        const key = !params.useRainbowNamingSchema
          ? `${params.storageKey}.${params.name}`
  ...
  const persistStorage: PersistStorage<PersistedState> = {
    ...
    setItem: async (name, value) => {
      await lazyPersist({
        ...
        serializer,
        storageKey,
        useRainbowNamingSchema: config.useRainbowNamingSchema,
        name,
        value,
      });
    },
...
export function defaultSerializeState<PersistedState>(
  ...
  try {
    return JSON.stringify(
      { state, version },
      shouldUseReplacer ? replacer : undefined,
    );
  } catch (error) {
    logger.error(
      new RainbowError(
        `[createRainbowStore]: Failed to serialize Rainbow store data`,
      ),
      { error },
    );
    throw error;
...
export function defaultDeserializeState<PersistedState>(
  ...
  try {
    ...
    logger.error(
      new RainbowError(
        `[createRainbowStore]: Failed to deserialize persisted Rainbow store data`,
      ),
      { error },
    );
    throw error;
