...
export const attachValueSubscriptionMap = new WeakMap<
  AttachValue<unknown>,
  Subscribe
>();
...
const storeSignalCache = new WeakMap<
  ...
  Map<
    ...
    Map<(a: unknown, b: unknown) => boolean, AttachValue<unknown>>
  >
>();
...
export function $(
  ...
  equalityFn: (a: unknown, b: unknown) => boolean = isEqual,
) {
...
function getOrCreateAttachValue<T, S>(
  store: StoreApi<T>,
  selector: (state: T) => S,
  equalityFn: (a: S, b: S) => boolean,
): AttachValue<S> {
  ...
  const existing = byEqFn.get(
    equalityFn as (a: unknown, b: unknown) => boolean,
  );
  if (existing) {
