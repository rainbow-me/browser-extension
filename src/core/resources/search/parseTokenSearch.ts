...
export function parseTokenSearchResults(assets: SearchAsset[]): SearchAsset[] {
  ...
  for (const asset of assets) {
    ...
    results.push({
      ...
      mainnetAddress: mainnetInfo ? mainnetInfo.address : chainId === ChainId.mainnet ? address : ('' as AddressOrEth),
      uniqueId,
    });
  }
    ...
    for (const asset of assets) {
      ...
      for (const chainIdString in assetNetworks) {
        ...
        results.push({
          ...
          mainnetAddress: mainnetInfo ? mainnetInfo.address : networkChainId === ChainId.mainnet ? address : ('' as AddressOrEth),
          uniqueId,
        });
      }
    }
