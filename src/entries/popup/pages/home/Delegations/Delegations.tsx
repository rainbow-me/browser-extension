...
export const Delegations = () => {
  ...
  const handleRevokeOne = useCallback(
    (delegation: ChainDelegation) => {
      ...
      const revokeReason = delegation.isThirdParty
  ...
  const { rainbowDelegations, thirdPartyDelegations } = useMemo(() => {
    const rainbow = delegations?.filter((d) => !d.isThirdParty) ?? [];
    const thirdParty = delegations?.filter((d) => d.isThirdParty) ?? [];
