...
describe('dApp send bug: no changes must not leak full wallet balance', () => {
  const fullBalanceAsset = ethWithBalance('10000', '20000000');

  test('no changes + full-balance asset → undefined', () => {
    const tx = makeTx({
      direction: 'out',
      type: 'send',
      changes: undefined,
      asset: fullBalanceAsset,
      value: '10000000000000000',
    });
    expect(getActivityDisplayValues(tx)).toBeUndefined();
  });

  test('empty changes array + full-balance asset → undefined', () => {
    const tx = makeTx({
      direction: 'out',
      type: 'send',
      changes: [],
  ...
  test('receive with no changes → undefined', () => {
    const tx = makeTx({
      direction: 'in',
      type: 'receive',
      changes: undefined,
  ...
  test('with proper changes → shows actual sent amount, not wallet balance', () => {
    const tx = makeTx({
      direction: 'out',
      type: 'send',
      changes: [{ direction: 'out', asset: ethWithBalance('0.01', '20') }],
    ...
    expect(result?.type).toBe('transfer');
    if (result?.type === 'transfer') {
      expect(result.topValue).toContain('0.01');
      expect(result.topValue).not.toContain('10,000');
      expect(result.topValue).not.toContain('10000');
    }
  });
});
...
describe('swap values', () => {
  test('returns out/in values from changes', () => {
    const tx = makeTx({
      type: 'swap',
      changes: [
  ...
  test.each(['swap', 'wrap', 'unwrap'] as const)(
    '%s type routes to swap handler',
    (type) => {
      const tx = makeTx({
        type,
        changes: [
  ...
  test('missing changes → undefined', () => {
    const tx = makeTx({ type: 'swap', changes: undefined });
    expect(getActivityDisplayValues(tx)).toBeUndefined();
  });

  test('missing in token → undefined', () => {
    const tx = makeTx({
      type: 'swap',
      changes: [{ direction: 'out', asset: ethWithBalance('1', '2000') }],
    });
  ...
  test('missing out token → undefined', () => {
    const tx = makeTx({
      type: 'swap',
      changes: [{ direction: 'in', asset: usdcWithBalance('2000', '2000') }],
    });
    expect(getActivityDisplayValues(tx)).toBeUndefined();
  });
});
