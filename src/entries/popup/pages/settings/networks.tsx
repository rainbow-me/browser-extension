...
const chainLabel = ({
  ...
  allChainLabels: Record<ChainId, string>;
  chainId: ChainId;
  testnet?: boolean;
}) => {
  const chainLabels = [
  ...
  if (allChainLabels[chainId]) {
    chainLabels.push(allChainLabels[chainId]);
  }
...
export function SettingsNetworks() {
  ...
  const [supportedChainIds] = useState(supportedChains.map(({ id }) => id));
  const allChainLabels = useBackendNetworksStore((state) =>
    ...
    () =>
      sortNetworks(userChainsOrder, mainChains).map((chain) => {
        ...
        return {
          ...chain,
          name:
            supportedChains.find(({ id }) => id === chainId)?.name ||
            chain.name,
        };
      }),
