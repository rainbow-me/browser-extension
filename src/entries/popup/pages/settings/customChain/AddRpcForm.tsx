...
export function AddRpcForm() {
  ...
  useEffect(() => {
    if (isChainIdMismatch) {
      triggerToast({
        title: i18n.t('settings.networks.custom_rpc.cant_connect'),
        description: `This RPC is for chain ID ${chainMetadata?.chainId}, not ${chain.name}`,
      });
    }
          ...
          <form.Field name="name">
            {(field) => (
              <FormInput
                ...
                placeholder={i18n.t('settings.networks.custom_rpc.rpc_name')}
