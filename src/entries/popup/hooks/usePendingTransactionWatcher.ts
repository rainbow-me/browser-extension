...
export function PendingTransactionWatcher() {
  ...
  const supportedChainIds = useMemo(() => {
    const userChainIds = new Set(chains.map(({ id }) => id));
    return getSupportedChains({ testnets: testnetMode })
      .map(({ id }) => id)
      .filter((id) => userChainIds.has(id));
  }, [chains, testnetMode]);
  ...
  const minedTransactionsKey = useMemo(
    () =>
      pendingTransactions
        .filter((tx) => tx.status !== 'pending')
        .map((tx) => `${tx.hash}:${tx.status}`)
        .join(','),
    [pendingTransactions],
  );
  ...
  const minedTransactions = useMemo(
    ...
    // Always rebuild together with minedTransactionsKey to trigger a re-render when the mined transactions change
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [minedTransactionsKey],
  );
  ...
  useEffect(() => {
    ...
    // use minedTransactionsKey as a stable dependency instead of the object array a dependency to trigger a re-render when the mined transactions change
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
    address,
    currentCurrency,
