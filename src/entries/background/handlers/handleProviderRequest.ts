  ...
  rnbwHandleProviderRequest({
    ...
    setBatch: (record) => {
      ...
      if (!session) {
        throw createProviderError(
          'INVALID_PARAMS',
          'No active session for this app',
        );
      }
      if (session.chainId !== validated.chainId) {
        throw createProviderError(
          'INVALID_PARAMS',
    ...
    showCallsStatus: async ({ chainId, tabId, batchId: id, ...params }) => {
      if (!getEip5792Enabled()) {
        throw createProviderError(
          'METHOD_NOT_SUPPORTED',
          'EIP-5792 methods are not enabled',
      ...
      const batch = useBatchStore.getState().getBatchByKey({ id, ...params });
      const txHash = batch?.receipts?.[0]?.transactionHash;
      if (txHash) {
